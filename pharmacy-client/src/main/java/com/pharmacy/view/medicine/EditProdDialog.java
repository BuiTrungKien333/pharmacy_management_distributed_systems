...
public class EditProdDialog extends javax.swing.JDialog {
    ...
    public EditProdDialog(Frame parent, boolean modal, MedicineResponse medicineResponse, MedicineUI medicineUI) {
    ...
    private void applyPermissions() {
        boolean ok = ClientSecurityContext.hasPermission("PRODUCT_EDIT");
    ...
    private void saveNewProd() {
        try {
            ...
            request.setDeletedAt(cmbStatus.getSelectedIndex() != 0 ? LocalDateTime.now() : null);
            ...
            if (tempAvatarFileName == null) {
                request.setAvatarUrl(this.medicineResponse.getAvatarUrl());
            } else {
                saveAvatarToPermanentLocation(barcode);
                request.setAvatarUrl(tempAvatarFileName);
            }
        ...
        medicineUI.showProduct(medicineService.getAllMedicineByPage(medicineUI.getCurrentPage()));
    ...
    private void loadDataProd() {
        if (medicineResponse == null) {
            ...
            lblAvatar.setIcon(HelperImageIcon.scaleIcon("/images/prod/default.png", WIDTH_IMG, HEIGHT_IMG));
            return;
        }

        txtMaSanPham.setText(String.format("%06d", medicineResponse.getId()));
        txtBarcode.setText(medicineResponse.getBarcode() == null ? "" : medicineResponse.getBarcode());
        txtTensp.setText(medicineResponse.getMedicineName() == null ? "" : medicineResponse.getMedicineName());

        txaHoatChat.setText("");
        cmbBaoChe.setSelectedIndex(-1);
        cmbDuongDung.setSelectedIndex(-1);
        cmbDonViTinh.setSelectedItem(medicineResponse.getMeasuringUnit());
        txtSoDangKi.setText("");
        cmbTieuChuan.setSelectedIndex(-1);
        cmbNuocSX.setSelectedIndex(-1);
        txtNhaSX.setText("");
        txtQuyCach.setText("");
        txaChiDinh.setText("");
        ...
        cmbStatus.setSelectedIndex(medicineResponse.getDeletedAt() == null ? 0 : 1);
        txtSoLuong.setText(String.valueOf(medicineResponse.getTotalQuantity()));

        int typeIndex = switch (medicineResponse.getMedicineType()) {
            case FUNCTIONAL_FOODS -> 1;
            case MEDICAL_INSTRUMENT -> 2;
            case MEDICINE, ALL -> 0;
        };
        cmbLoai.setSelectedIndex(typeIndex);

        String avatarPath = medicineResponse.getAvatarUrl();
        lblAvatar.setIcon(HelperImageIcon.scaleIcon(
                (avatarPath == null || avatarPath.isBlank()) ? "/images/prod/default.png" : avatarPath,
                WIDTH_IMG,
                HEIGHT_IMG
        ));
    }
