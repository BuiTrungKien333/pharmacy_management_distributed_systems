package com.pharmacy.view.menu.mode;
...
public class LightDarkMode extends JPanel {
	...
	private void init() {
		...
		buttonLight = new JButton("Light", new FlatSVGIcon("icon/mode/light.svg"));
		buttonDark = new JButton("Dark", new FlatSVGIcon("icon/mode/dark.svg"));
	...
	private void checkStyle() {
		...
		if (isDark)
			buttonLighDark.setIcon(new FlatSVGIcon("icon/mode/dark.svg"));
		else
			buttonLighDark.setIcon(new FlatSVGIcon("icon/mode/light.svg"));
	}
	...
	private void addStyle(JButton button, boolean style) {
		if (style) {
			...
			button.putClientProperty(FlatClientProperties.STYLE,
					"" + "arc:999;" + "background:$Menu.lightdark.button.background;" + "foreground:$Menu.foreground;"
							+ "focusWidth:0;" + "borderWidth:0;" + "innerFocusWidth:0;" + "background:null");
		}
	}
